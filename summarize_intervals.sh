...
INDIR="${INDIR:-perf_data/intervals}"
WARMUP="${WARMUP:-12}"
TRIM="${TRIM:-3}"
SCRIPT_DIR="${SCRIPT_DIR:-scripts}"
AWK_ANALYZER="$SCRIPT_DIR/perf_stats_v3.awk"

mkdir -p "$SCRIPT_DIR"

cat > "$AWK_ANALYZER" <<'AWK'
BEGIN { FS=","; saw_time=0 }
...
stats_out="${INDIR}/_summaries"
mkdir -p "$stats_out"

shopt -s nullglob
mapfile -t csvs < <(printf "%s\n" "${INDIR}"/perf_fp*.csv)
if (( ${#csvs[@]} == 0 )); then
  echo "No perf CSV files found under ${INDIR}. Check INDIR and filenames."
  exit 1
fi

for csv in "${csvs[@]}"; do
  base="$(basename "$csv" .csv)"
  out="${stats_out}/${base}.stats"
  echo "Summarizing $csv → $out"
  WARMUP="$WARMUP" TRIM="$TRIM" awk -f "$AWK_ANALYZER" "$csv" | tee "$out" >/dev/null
done

echo "Done. Per-run summaries are under ${stats_out}."
